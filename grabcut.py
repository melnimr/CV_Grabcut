...
def GrabCut(filename,T,iter,N_com,gamma):
	#reading image data
	...
	#initalize GMM
	gmm = [mixture.GMM(n_components=N_com),mixture.GMM(n_components=N_com)]#two GMMs(background and foreground) with 5 components
	...
	result.save("segment_output_" + str(iter)+ "_" + str(gamma)+ "_" + str(N_com)+ ".png")
	return result
